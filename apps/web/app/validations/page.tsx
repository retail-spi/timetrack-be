          ...
          {entries.map((e) => (
            <div key={e.id} className={`${card} p-4`}>
              <div className="mb-3">
                ...
                <p className={`text-[12px] mt-0.5 ${muted}`}>{new Date(e.startTime).toLocaleTimeString('fr-BE', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Brussels' })} → {new Date(e.endTime).toLocaleTimeString('fr-BE', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Brussels' })}{e.breakMinutes > 0 && ` · pause ${e.breakMinutes} min`}</p>
              </div>
